...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if userCtx.Role == roles.Admin {
		if id := event.QueryStringParameters["userId"]; id != "" {
			user, userErr := userService.GetUserById(ctx, id)
			if userErr != nil {
				err = userErr
			} else {
				res = append(res, user)
			...
			res, err = userService.GetAllUsers(ctx)
		}
	} else {
		user, userErr := userService.GetUserById(ctx, userCtx.ID)
		if userErr != nil {
			err = userErr
		} else {
			res = append(res, user)
