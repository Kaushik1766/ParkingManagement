...
func TestOfficeService_GetOfficeByName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := officeServ.GetOfficeByName(tt.args.ctx, tt.args.officeName)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetOfficeByName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("GetOfficeByName() got = %v, want %v", got, tt.want)
			}
		})
	}
}
