...
func (sqlpr *SQLParkingRepository) GetParkingHistoryByUser(userId string, startTime time.Time, endTime time.Time) ([]models.ParkingHistoryDTO, error) {
	var history []models.ParkingHistory
	err := sqlpr.db.Joins("Vehicle").Where("Vehicle.user_id = ? AND start_time >= ? AND end_time <= ? AND end_time IS NOT NULL", uuid.MustParse(userId), startTime, endTime).Find(&history).Error
	if err != nil {
		return nil, err
	}
	var historyDTO []models.ParkingHistoryDTO
...
func (sqlpr *SQLParkingRepository) GetActiveUserParkings(userId string) ([]models.ParkingHistoryDTO, error) {
	...
	err := sqlpr.db.Joins("Vehicle").Where("Vehicle.user_id = ? AND end_time IS NULL", uuid.MustParse(userId)).Find(&activeParkings).Error
	if err != nil {
		return nil, err
	}
